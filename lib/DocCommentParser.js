...
DocCommentParser.prototype = {
  ...
  _extractAllLineUntilNextTag: function () {
    ...
    targetField = this.tagDef.enableMultiTag ? this.data[tag][this.mutliTagSubTarget.key].value.description : this.data[tag];
    ...
    while (this.line !== void 0 && !(match && tagDef)) {
      ...
      if (this.tagDef.enableMultiTag) {
        ...
        this.data[tag][this.mutliTagSubTarget.key].value.description += (targetField.length ? '\n' : '') + this.line;
      } else {
        this.data[tag] += (targetField.length ? '\n' : '') + this.line;
      }
  ...
  _saveData: function (value) {
    if (this.tagDef.enableMultiTag) {
      this.data[this.tagDef.docProperty][this.mutliTagSubTarget.key] = value;
    } else {
  ...
  _extractLine: function _extractLine() {
    ...
    if (this.match && this.tagDef) {
      if (this.tagDef.enableMultiTag) {
        this.mutliTagSubTarget = this.tagDef.parse(this.match[2]);
